...
**vEncoder** is optional. It is the encoder the plugin will use for camera streams. If vEncoder is not specified it will default to "libx264 -preset ultrafast -tune zerolatency". You can use "copy" to pass the camera's H.264 stream through directly without transcoding. This requires almost no CPU and is highly recommended for low-power setups. When `"copy"` is selected, the plugin automatically advertises only the camera's native stream resolution to HomeKit to ensure perfect resolution matching and prevent black screens. On a Raspberry Pi 4 you can try something like "h264_v4l2m2m". If you don't know what this means you can probably ignore it.
